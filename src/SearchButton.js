    ...
    useEffect(() => {
        ...
        if (isMounted.current) {
            const getTranslation = async () => {
                const firstRequest = await fetch(`https://cors-anywhere.herokuapp.com/https://api.collinsdictionary.com/api/v1/dictionaries/${dictionaryValue}/search/first/?q=${search}`, {
                    method: 'GET', 
                    headers: {
