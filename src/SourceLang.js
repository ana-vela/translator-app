...
function SourceLang() {
  ...
  React.useEffect(() => {
    async function getLang() {
      const response = await fetch('https://dictapi.lexicala.com/languages', {method: 'GET',
    headers: headers});
    const body = await response.json();
    setItems(Object.keys(body.language_names).map(language_names  => ({ label: language_names, value: language_names })));
  }
  getLang();
}, []);
